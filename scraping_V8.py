#Esta versión incluye un apartado de filtros para poder seleccionar año y palabras clave    ///FUNCIONAL///
import pandas as pd
import requests
import os
...
class ScraperApp:
    def __init__(self, root):
        ...
        tk.Label(self.tab_files, text= " Descargar archivos CSV, XLSX, PDF Y ZIP ", font = ("Arial", 12, "bold")).pack(pady=(15,5))
        ...
        tk.Button(frame_filtros, text="Ver todos", command=self.limpiar_filtros, bg = "#DDDDDD" ).pack(side="left")
    ...
    #Función de filtro y enlistar
    def actualizar_lista_visual(self, lista_datos):         #Limpia interfaz
        ...
        for i in indices_seleccionados:                                # Iteramos sobre los seleccionados
            ...
            try:
                ...
                if not ruta_completa:                   # Si el usuario da "Cancelar" en la ventana de guardar, ruta_completa estará vacía
                    continue                            # Simplemente saltamos este archivo y seguimos con el siguiente (si hubiera más)
                ...
                # Headers SE PUEDE QUITAR SI NO SALE XD 
                headers = {'User-Agent': 'Mozilla/5.0'}
                #headers = {'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/91.0.4472.124 Safari/537.36'}     #PRUEBAAA
