#Se hizo una modificación y ya no abre el navegador en el apartado de "buscar archivos."                /// F U N C I O N A L 
#Se añadión una casilla de "Seleccionar todos", donde, depende la cantidad de archivos, pide nombre o descarga directo. Además de pedir verificación
import pandas as pd
import requests
import os
...
class ScraperApp:
    def __init__(self, root):
        ...
        tk.Label(self.tab_files, text= " Descargar archivos CSV, XLSX Y PDF", font = ("Arial", 12, "bold")).pack(pady=(15,5))
        ...
        tk.Label(frame_filtros, text= " Año ").pack(side = "left", padx= 5)
        self.entry_filtro_anio = tk.Entry(frame_filtros, width=10)
        self.entry_filtro_anio.pack(side = "left", padx=5)

        tk.Label(frame_filtros, text= " Texto ").pack(side = "left", padx= 5)
        self.entry_filtro_texto = tk.Entry(frame_filtros, width=20)
        self.entry_filtro_texto.pack(side = "left", padx=5)

        tk.Button(frame_filtros, text="Aplicar filtros", command= self.aplicar_filtros, bg="#DDDDDD").pack(side="left", padx=10)
        tk.Button(frame_filtros, text="Eliminar filtros", command=self.limpiar_filtros, bg = "#DDDDDD" ).pack(side="left")
    ...
    def escanear_archivos(self):
        try:
            ...
            ext_interes = ('.csv', '.xlsx', '.pdf')                     #Exensiones de interés MODIFICABLE
            ...
            for link in enlaces:                                                             #Búsqueda de info
                try:
                    ...
                    if url and (url.lower().endswith(ext_interes) or "download" in url.lower() or "descargar" in texto.lower()):
                        ...
                        chk = tk.Checkbutton(self.scroll_frame, text = f"{texto} \n({nombre_archivo_completo})", variable=var, bg="white", justify="left", anchor="w")
    ...
    def aplicar_filtros(self):
        filtro_anio = self.entry_filtro_anio.get().strip()
        filtro_texto = self.entry_filtro_texto.get().strip().lower()
        ...
        for texto, url in self.enlaces_encontrados:
            ...
            if filtro_texto:
                if filtro_texto not in texto.lower() and filtro_texto not in url.lower():        #Se busca que esté la palabra
                    cumple_texto = False
            if cumple_anio and cumple_texto:
    ...
    def descargar_archivos_seleccionados(self):                                                  #Función para descargar los archivos que eligió el usuario
        ...
        if modo_descarga == "AUTOMATICO":
            ...
            if not respuesta: #Si se arrepiente y cancela
                ...
                carpeta_destino = filedialog.askdirectory(title = "Selecciona la carpeta donde quieres descargar tus archivos.")
        ...
        for i in indices_seleccionados:                                                          #Iteramos sobre los seleccionados
            ...
            try:
                ...
                #Cambios para la ruta de guardado dependiendo el MODO
                ruta_completa = ""

                if modo_descarga == "MANUAL":                       #Descarga MODO MANUAL
