#   / / /   V e r s i o n   2 0 :   F u n c i o n a l
...
class ScraperApp:
    def __init__(self, root):
        ...
        try:                                                                    #Ahora usa ruta_recurso para que no busque en la carpeta actual, para que sirva en el .exe
            image_path = ruta_recurso("reportes_incidencia.jpeg")               #Busca imagen para el botón 1
        ...
        try:
            image_path_datos = ruta_recurso("datos_abiertos.jpeg")              #Busca imagen para el botón 2
        ...
        tk.Label(self.tab_conversor, text="Esperando archivo. . .", font=("Arial", 8), fg="gray").pack(pady=(10,20))
        tk.Label(self.tab_conversor, text="Considera que los archivos CSV con una dimensión mayor a las permitidas \n en excel puede dar errores al hacer la conversión.", font=("Arial", 12)).pack(pady=(25,55))
    ...
    def descargar_archivos_seleccionados(self):
        ...
        nube_links = [u for u in urls_a_descargar if any(x in u.lower() for x in ["onedrive", "sharepoint", "idrv.ms"])]         #Verificación en NUBE: Se abrirá en el navegador
        ...
        if nube_links:                                                                                                           #Mensaje advirtiendo que se abrirá el navegador
            mensaje = f"Has seleccionado {len(nube_links)} enlace(s).\n\n" \
                      "Se te pedirá la carpeta para poder descargar los archivos primero.\n" \
                      "Se abrirá un navegador automático para descargar estos archivos.\n\n¿Continuar?"
            
            if messagebox.askyesno("Enlcaes de Nube", mensaje):
                carpeta_nube = filedialog.askdirectory(title="Selecciona dónde descargar los archivos")     #Pedir directorio 

                if carpeta_nube:
                    chrome_prefs ={
                        "download.default_directory": carpeta_nube.replace("/", "\\"),
                        "download.prompt_for_download": False,
                        "safebrowsing.enabled": True
                    }
                    opciones_nube = Options()
                    opciones_nube.add_experimental_option("prefs", chrome_prefs)
                    opciones_nube.add_argument("--no-sandbox")
                    opciones_nube.add_argument("--disable-dev-shm-usage")
                    opciones_nube.add_argument("--disable-gpu")
                    opciones_nube.add_argument("--start-maximized")                                         #Visible para ver la descarga en la esquina de arriba ||Opcional||

                    try:
                        driver_nube = webdriver.Chrome(service=Service(ChromeDriverManager().install()), options=opciones_nube)             #Iniciar driver temp
                        for link in nube_links:                                                                                             #Iterar y descargar
                            link_descarga = link                                                                                            #Forzar descarga
                            if "download=1" not in link:
                                if "?" in link: link_descarga += "&download=1"
                                else: link_descarga += "?download=1"
                            driver_nube.get(link_descarga)
                            time.sleep(5)                                                                                                   #Pausa entre archivos

                    except Exception as e:
                        messagebox.showerror("Error de librería", f"No se pudo iniciar la descarga: \n{e}")
    ...
    def procesar_verificacion(self, df):
        ...
        def check_with_selenium(url):
            nonlocal driver_oculto
            if not driver_oculto:
                opts = Options()
                opts.add_argument("--headless=new"); opts.add_argument("--no-sandbox"); opts.add_argument("--disable-dev-shm-usage"); opts.add_argument(f"user-agent={self.user_agent_usado}")
                driver_oculto = webdriver.Chrome(service=Service(ChromeDriverManager().install()), options=opts)
            try:
                driver_oculto.get(url)
                if "404" in driver_oculto.title or "Error" in driver_oculto.title: return "Inválido"
                return "Válido (Recuperado)"
        ...
        for link in lista_unica:
            ...
            try:
                ...
                if r.status_code == 200:
                    ...
                    if r.status_code == 200:
                        estado = check_with_selenium(str(link))                     #Válido
                else:
                    r = requests.get(str(link), headers=headers, timeout=5, stream=True)
                    if r.status_code == 200: 
                        estado = check_with_selenium(str(link))                     #Válido
            except:
                estado = check_with_selenium(str(link))                             #Invalido
