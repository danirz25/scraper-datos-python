""" Se trabaja con la pestaña de verificador de enlaces 
        Se marca por colores si es válido o no
        Sugiere un nombre para guardar el archivo
"""
...
class ScraperApp:
    ...
    def convertir_archivos(self):
        print("Se está trabajando en esto")
    ...
    def procesar_verificacion(self, df):
        ...
        for i, link in enumerate(df['Link']):
            ...
            if pd.isna(link) or str(link). strip() == "":
                ...
                try:
                    r = requests.head(str(link), headers=headers, timeout=5, allow_redirects=True)      #Para rapidez se usa head, para certeza usamos get
                    if r.status_code == 200:                                                            #Si la pagina responda / existe
                        estado = "Valido"
                        ...
                        r = requests.get(str(link), headers=headers, timeout=5, stream=True)            #2 intentos de get para que sea más preciso
                        if r.status_code == 200: 
                            estado = "Valido"
                except:
                    estado = "Invalido"
            resultados.append(estado)
        df_temp = pd.DataFrame({"Link Original": df['Link'], "Estado": resultados})

        df_filtrado = df_temp[df_temp["Estado"] != "Sin enlace"]                        #Se filtran las filas sin enlace
        self.root.after(0,self.finalizar_verificacion, df_filtrado)
