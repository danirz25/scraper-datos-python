"""
Botón para ir directo al SESNSP. No permite repetir link en las búsquedas de forma consecutiva 
    - Igual, si no encuentra archivos pregunta si quiere volver a intentar
"""
...
class ScraperApp:
    def __init__(self, root):
        ...
        self.icon_sesnsp = None                             #Icono de la imagen
        try:
            image_path = "reportes_incidencia.jpeg"         #Busca la imagen para el botón
        ...
        tk.Button(frame_botones_search, text=" Buscar archivos ", command= lambda: self.abrir_navegador("ARCHIVOS"), bg = "#673AB7", fg="white").pack(side="left", pady=5)     #MODO: "ARCHIVOS"
        
        #Botón de acceso directo a SESNP
        if self.icon_sesnsp:
            btn_sesnsp = tk.Button(frame_botones_search, text="Incidencia delictiva", image= self.icon_sesnsp, compound="bottom", command=self.acceso_directo_sesnsp, cursor="hand2")
    ...
    def abrir_navegador(self, modo):
        ...
        if modo == "ARCHIVOS":
            if url== self.ultima_url_buscada:
                messagebox.showinfo("Información", "Ya se están mostrando los resultados. \n No hace falta recargar")
                return
            else:
                self.ultima_url_buscada = url                           #Actualizamos la útlima búsqueda (por si es nueva)
        ...
        try:
            ...
            self.ultima_url_buscada = ""                                                            #Reseteamos 
            if self.driver: 
        ...
        try:
            ...
            if contador > 0:
                ...
                for widget in self.scroll_frame.winfo_children():
                    widget.destroy()
                respuesta = messagebox.askyesno("Sin resultados", "No se hallaron resultados. \n ¿Quieres volver a intentarlo?\n A veces la página puede tardar en responder")
                if respuesta:
                    self.ultima_url_buscada =""
                    self.root.after(500, lambda: self.abrir_navegador("ARCHIVOS"))
